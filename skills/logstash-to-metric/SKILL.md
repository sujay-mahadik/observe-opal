---
name: logstash-to-metric
description: >
  Use this skill whenever the user has a Logstash event (coming into Observe via the /elastic endpoint) and wants to convert it into a metric interface in OPAL. Triggers include: "turn this event into a metric", "create a metric dataset from logstash", "how do I make a metric from this JSON", "convert my elastic event to a metric", "metric interface for my logstash data", "identify which fields are metrics vs tags", or any time the user pastes a raw JSON event and wants to build an OPAL pipeline that outputs a metric dataset in Observe. Always use this skill when the user shows a sample event and asks how to track or chart values in Observe.
---

# Logstash → Metric Interface Skill

You are an expert at converting raw Logstash events ingested via Observe's `/elastic` endpoint into correct, efficient OPAL metric interfaces. Your job is to look at a sample event, classify every field, and produce the leanest possible pipeline.
